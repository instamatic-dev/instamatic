...
def calibrate_dose(ctrl, num_imgs: int = 10, threshold: int = 50, logger=logger):
    ...
    if ctrl.tem.name[:3] == 'fei':
        ...
        e_per_pixel_per_reading_per_sec = ctrl.current_density * 1e-12 / avg_reading * 6.242e18 * 
                                (ctrl.cam.cam.binsize*ctrl.cam.cam.physical_pixelsize*0.1)**2
...
def convert_to_dose_rate(ctrl, num_imgs: int = 10, img_fn: str = None, e_per_pixel_per_reading_per_sec: float = None):
    ...
    if img_fn is None:
        ...
        e_per_Angstrom2_per_second = e_per_pixel_per_reading_per_sec * img.mean() / 
                                (ctrl.cam.cam.binsize*ctrl.cam.cam.physical_pixelsize*1e7)**2
    else:
        img, h = read_image(img_fn)
        e_per_Angstrom2_per_second = e_per_pixel_per_reading_per_sec * img.mean() / 
                                (config.camera.default_binsize*config.camera.physical_pixelsize*1e7)**2
