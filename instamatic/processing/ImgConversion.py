...
class ImgConversion:
    ...
    def write_pets2_inp(self, path: str, tiff_path: str = 'tiff') -> None:
        ...
        with open(path / 'pets.pts2', 'w') as f:
            date = str(time.ctime())
            print(f'# PETS 2 input file for Rotation Electron Diffraction generated by `instamatic`', file=f)
            print(f'# {date}', file=f)
            print(f'# For definitions of input parameters, see:', file=f)
            print(f'# http://pets.fzu.cz/ ', file=f)
            print(f'', file=f)
            print(f'geometry continuous', file=f)
            print(f'lambda {self.wavelength}', file=f)
            print(f'Aperpixel {self.pixelsize}', file=f)
            print(f'phi {float(self.osc_angle)/2}', file=f)
            print(f'omega {omega}', file=f)
            print(f'bin 1', file=f)
            print(f'reflectionsize 15', file=f)
            print(f'noiseparameters 25 10', file=f)
            print(f'i/sigma    5.00   10.00', file=f)
            print(f'', file=f)
            # print("reconstructions", file=f)
            # print("endreconstructions", file=f)
            # print("", file=f)
