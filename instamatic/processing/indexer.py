...
def merge_csv(csvs):
    return pd.concat((read_csv(csv) for csv in csvs))


def multi_run(arg, procs=1, dry_run=False, resize=False):
    ...
    for i in xrange(procs):
        print " >> Proc #{}: {} files".format(len(fn[i::procs]))
        root, ext = os.path.splitext(csv_out)
        csv_outs.append("{}_{}{}".format(root, offset, ext))
    print
    ...
    if resize: # resize window to fit output
    ...
    all_results = merge_csv(csv_outs)
    if len(all_results) > 0:
        write_csv(csv_out, all_results)
        print "Writing results to {}".format(csv_out)
    print
